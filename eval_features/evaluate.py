...
PREDICTION_PATH = r'/home/users/zzweng/SimCLR/output/inference/lvis_instances_results.json'
...
LVIS_API_PATH = r'/home/users/zzweng/SimCLR/lvis-api'
...
class LVISEvaluator(object):
    ...
    def _save_gt_features(self):
        ...
        from .feature_saver import LvisSaver
        import torch
        config = self.config
    ...
    def _save_dt_features(self):
        from eval_features.feature_saver import LvisSaver
        import torch
        config = self.config
        if self.config['hyperbolic']:
            from models.hyperbolic_resnet import HResNetSimCLR
        ...
        model.load_state_dict(state_dict)
        model.eval()
        saver = LvisSaver(model, self.lvis_dt, DT_FEATS)
        saver.save()
